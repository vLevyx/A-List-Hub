...
export default function PricePlannerPage() {
  ...
  const calculateComponentCost = (name: string, qty: number = 1): number => {
    ...
    if (componentRecipes[name]) {
      return Object.entries(componentRecipes[name]).reduce(
        (sum, [sub, count]) => {
  ...
  const breakdownResources = (
    ...
    const recurse = (name: string, qty: number) => {
      if (
        rawMaterialPrices[name] !== undefined ||
        customPrices[name] !== undefined
      ) {
  ...
  const calculateComponentOnly = () => {
    if (!selectedComponent || componentQuantity < 1) {
      setResult({
        error: "Please select a component and enter a valid quantity.",
      });
      return;
    }

    const baseCost = calculateComponentCost(
      selectedComponent,
      componentQuantity
    );
    const multiplier = priceMultiplier / 100;
    const totalCost = baseCost * multiplier;
    const breakdown = breakdownResources({
      [selectedComponent]: componentQuantity,
    });
    ...
    <div className="min-h-screen bg-[#121212] text-[#e5e5e5]">
      ...
      >
        <div className="p-8 border-b border-white/10">
          <h3 className="text-[#00c6ff] text-2xl font-semibold mb-2">
            ...
            {Object.entries(rawMaterialPrices).map(([name, defaultPrice]) => (
              ...
              >
                ...
                <div className="flex items-center bg-white/5 border border-white/20 rounded overflow-hidden">
                  <span className="px-2 py-1 bg-white/5 border-r border-white/10 text-white/90">
                    $
                  </span>
                  <input
                    type="number"
      ...
      <div className="container max-w-7xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          ...
          <p className="text-[#a0a0a0] text-lg">
            Estimate item value through raw material analysis.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white/5 rounded-2xl p-8 border border-white/10 backdrop-blur-lg">
            <h2 className="text-2xl font-semibold mb-6 text-[#00c6ff]">
              Item Calculator
            </h2>
            ...
            <div className="space-y-6">
              ...
              <div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-semibold ${
                        multiplierStatus.className === "status-base"
                          ? "bg-yellow-500/20 text-yellow-400 border border-yellow-500/30"
                          : multiplierStatus.className === "status-lower"
                ...
                <input
                  ...
                  style={{
                    backgroundImage: `linear-gradient(to right, rgba(0, 198, 255, 0.5) 0%, rgba(0, 198, 255, 0.5) ${
                      priceMultiplier / 2
            ...
            {result.error ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                  ...
                  <div className="bg-white/5 p-4 rounded-lg text-center">
                    <div className="text-[#a0a0a0] text-sm mb-1">
                      Labor Cost ({result.hours}h × ${result.rate})
                    </div>
                    <div className="text-2xl font-bold text-[#10b981]">
                ...
                <div className="bg-white/[0.03] rounded-lg p-6">
                  <h4 className="text-xl font-semibold mb-4 text-[#ffc107]">
                    Raw Material Breakdown
                  </h4>
                  <ul className="space-y-2">
                    {Object.entries(result.breakdown).map(([name, qty]) => {
                      const price =
                        customPrices[name] || rawMaterialPrices[name];
                      ...
                      return (
                        ...
                        >
                          <span className="text-white/90">
                            {totalQty} × {name} @ ${price.toLocaleString()}
                          </span>
                  ...
                  <ul className="space-y-2">
                    {Object.entries(result.breakdown).map(([name, qty]) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-white/90">
                            {qty as number} × {name} @ ${price.toLocaleString()}
                          </span>
                          <span className="text-[#10b981] font-semibold">
                            ${itemCost.toLocaleString()}
                          </span>
