...
const STRATEGY_DESCRIPTIONS = {
  balanced:
    "Will balance shortest distance and parcel efficiency, creating an optimal route that considers both travel time and delivery priorities.",
  distance:
    "Will calculate the shortest distance / quickest time to complete visiting all selected stations.",
  efficiency:
    "Will prioritize stations with the most current deliveries first. When delivery counts are equal, it switches to a balanced route strategy for optimal efficiency.",
} as const;
...
function calculateStraightLineDistance(
  station1: ParcelStation,
  station2: ParcelStation
): number {
  ...
  while (improved && iterations < maxIterations) {
    ...
    for (let i = 1; i < bestRoute.length - 2; i++) {
      for (let j = i + 1; j < bestRoute.length; j++) {
        ...
        if (
          calculateTotalRouteTime(newRoute) < calculateTotalRouteTime(bestRoute)
        ) {
          bestRoute = newRoute;
          improved = true;
        }
...
function optimizeRoute(
  ...
  const stationsToVisit = deliveries.map(
    (job) => PARCEL_STATIONS.find((station) => station.id === job.stationId)!
  );
  ...
  while (unvisited.length > 0) {
    ...
    if (settings.strategy === "distance") {
      ...
      nextStation = unvisited.reduce((best, station) => {
        const stationJob = deliveries.find(
          (job) => job.stationId === station.id
        )!;
  ...
  const optimizedOrder = twoOptImprovement([
    currentStation,
    ...visitOrder,
  ]).slice(1);
  const optimizedTotalTime = calculateTotalRouteTime([
    currentStation,
    ...optimizedOrder,
  ]);

  // Create a single "trip" representing the optimal visiting sequence
  const totalParcels = deliveries.reduce(
    (sum, job) => sum + job.parcelCount,
    0
  );
  const singleTrip = {
...
export default function ParcelRoutePlanner() {
  ...
  // ===== COMPUTED VALUES =====
  const currentStation = PARCEL_STATIONS.find(
  ...
  const selectedStations = deliveries.map(
    (job) => PARCEL_STATIONS.find((s) => s.id === job.stationId)!
  );
  ...
  const handleParcelCountChange = (stationId: number, count: number) => {
    if (count <= 0) {
      setDeliveries((prev) =>
        prev.filter((job) => job.stationId !== stationId)
      );
    } else {
      setDeliveries((prev) =>
        prev.map((job) =>
        ...
        .map((job) => {
          if (job.stationId === stationId) {
            const newCount = Math.max(
              0,
              Math.min(50, job.parcelCount + increment)
            );
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setCurrentStationId(Number(e.target.value))
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSettings((prev) => ({
                          ...prev,
                          strategy: e.target
                            .value as OptimizationSettings["strategy"],
                        }))
                      }
                      className="w-full bg-background-tertiary border border-white/20 rounded-lg px-3 py-2 text-white focus:border-primary-500 focus:outline-none"
                    >
              ...
              <div className="bg-white/[0.02] border border-white/[0.08] rounded-xl p-6 backdrop-blur-lg">
                ...
                <p className="text-white/60 text-sm mb-4">
                  Select stations where you have parcels to deliver. You'll pick
                  up new parcels at each station you visit.
                </p>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {PARCEL_STATIONS.filter(
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-white/60 text-sm">
                          Grid: {station.gridX.toString().padStart(3, "0")}/
                          {station.gridZ.toString().padStart(3, "0")}
                        </p>
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <div className="flex items-center gap-1">
                                <button
                                  onClick={() =>
                                    handleParcelIncrement(station.id, -1)
                                  }
            ...
            >
                            ...
                            >
                              ...
                              <div className="text-white/60 text-sm">
                                {index === 0
                                  ...
                                  : `${getRealTravelTime(
                                      routeResult.stations[index - 1].id,
                                      station.id
                                    )}s`}
                              </div>
                            </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-white/10">
                        ...
                        <div className="space-y-2">
                          {deliveries.map((delivery) => {
                            const station = PARCEL_STATIONS.find(
                              (s) => s.id === delivery.stationId
                            )!;
                            return (
                              <div
                                key={delivery.stationId}
                                className="flex items-center justify-between bg-white/5 rounded px-3 py-2"
                              >
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    onClick={() =>
                                      handleDelivered(delivery.stationId)
                                    }
              ...
              <div className="bg-white/[0.02] border border-white/[0.08] rounded-xl p-6 backdrop-blur-lg">
                ...
                <ul className="space-y-2 text-white/70 text-sm">
                  ...
                  <li className="flex items-start gap-2">
                    <div className="w-1 h-1 bg-primary-500 rounded-full mt-2 flex-shrink-0" />
                    You'll pick up new parcels at each station you visit along
                    the route
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="w-1 h-1 bg-primary-500 rounded-full mt-2 flex-shrink-0" />
                    Use the "Visited" button to update your route as you
                    complete deliveries
                  </li>
                  <li className="flex items-start gap-2">
                    <div className="w-1 h-1 bg-primary-500 rounded-full mt-2 flex-shrink-0" />
                    "Delivery Priority" strategy works best when you have
